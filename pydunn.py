...
def inter_cluster_distances(
    ...
    cluster_pairs = (
        (c1, c2) for i, c1 in enumerate(c_labels) for c2 in c_labels[i + 1 :]
    )
...
def compute_cluster_diameters(
    ...
    if method == DiameterMethod.MEAN_CLUSTER:
        ...
        diameters = {
            c: distances[labels == c][:, labels == c].sum()
    ...
    # for farthest cluster diameter method, get the maximum distance between all data points in the same cluster
        ...
        diameters = {
            c: distances[labels == c][:, labels == c].max()
            for c in np.unique(labels)
        }
...
def dunn(
    ...
    label_map = {
        old_label: new_label for new_label, old_label in enumerate(set(labels))
    }
    ...
    max_diameter = max(
        compute_cluster_diameters(labels, distances, diameter_method).values()
    )
...
def __validate_distance_matrix(distances: np.ndarray) -> None:
    ...
    assert np.allclose(
        distances, distances.T, rtol=1e-05, atol=1e-08
